...
ShellRoot {
    ...
    property string malwareAdvice: "sudo pacman -S clamav && sudo freshclam && sudo systemctl enable --now clamav-freshclam.service clamav-daemon.service clamav-clamonacc.service"

    // ─── Lynis Audit Properties ───────────────────────
    property bool   lynisInstalled: false
    ...
    // ─── Malware Check Process ────────────────────────
    Process {
        id: malwareProc
        command: ["sh", "-c", "if pacman -Q clamav >/dev/null 2>&1; then echo 'installed'; else echo 'not_installed'; fi"]

        stdout: SplitParser {
            onRead: function(line) {
                if (line) {
                    clamInstalled = (line.trim() === "installed")
                }
            }
        }
    }
    ...
    Process {
        id: lynisCheckProc
        command: ["sh", "-c", "if command -v lynis >/dev/null 2>&1; then echo 'installed'; else echo 'not_installed'; fi"]

        stdout: SplitParser {
            onRead: function(line) {
                if (line) {
                    lynisInstalled = (line.trim() === "installed")
                }
            }
        }
    }
    ...
    // ─── Launcher for Lynis execution in Kitty terminal ───
    Process {
        id: openScanOSD
        command: [
            "kitty",
            ...
            ScrollView {
                ...
                Column {
                    ...
                    Text {
                        width: parent.width
                        horizontalAlignment: Text.AlignHCenter
                        visible: !lynisInstalled
                        text: "Lynis not installed\nCommand: sudo pacman -S lynis"
                        color: "#ffaaaa"
                        font.pixelSize: 14
                        font.family: "Monospace"
                    }
                    ...
                    Button {
                        ...
                        onClicked: {
                            if (lynisInstalled) {
                                ...
                                copyProcess.command = ["wl-copy", "sudo pacman -S lynis"]
                                copyProcess.running = false
                                copyProcess.running = true
                            }
                        }
